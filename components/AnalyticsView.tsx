  ...
  return (
    <div className="animate-slide-up space-y-6 pb-20">
        ...
        <div className={`p-8 rounded-3xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6
             ${isInnovation ? 'glass-prism bg-gradient-to-br from-white to-blue-50' : isOcean ? 'bg-ocean-neon/5 border border-ocean-neon/40 backdrop-blur-2xl shadow-[0_0_30px_rgba(0,243,255,0.1)]' : isTactical ? 'bg-black/60 border border-ops-green/40 shadow-[0_0_30px_rgba(57,255,20,0.1)]' : styles.cardClass}
        `}>
        ...
        {loading && (
            <div className={`p-12 text-center rounded-3xl ${styles.cardClass} animate-pulse`}>
                <div className={`inline-block p-6 rounded-full mb-6 ${isOcean ? 'bg-ocean-neon/10 text-ocean-neon' : isTactical ? 'bg-ops-green/10 text-ops-green' : 'bg-gray-100'}`}>
        ...
        {report && !loading && (
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div className={`p-8 rounded-3xl overflow-y-auto max-h-[800px] custom-scrollbar
                    ${isInnovation ? 'bg-white shadow-lg border border-gray-100' : isOcean ? 'bg-black/40 border border-ocean-neon/30 backdrop-blur-md' : isTactical ? 'bg-black/40 border border-ops-green/30' : styles.cardClass}
                `}>
                    <h3 className={`text-xl font-bold mb-6 flex items-center gap-2 border-b pb-4 ${isOcean ? 'border-ocean-neon/30 text-ocean-neon' : isTactical ? 'border-ops-green/30 text-ops-green' : 'border-gray-200 text-gray-800'}`}>
                        <i className="fas fa-file-alt"></i> รายงานสรุปสำหรับผู้บริหาร
                    </h3>
                    <div className={`prose max-w-none whitespace-pre-line leading-relaxed ${isOcean ? 'text-ocean-neon/90' : isTactical ? 'text-ops-green/90' : 'text-gray-700'}`}>
                        {report}
                    </div>
                </div>
                ...
                <div className={`p-8 rounded-3xl flex flex-col
                    ${isInnovation ? 'bg-gray-900 text-gray-100' : isOcean ? 'bg-black/80 border border-ocean-neon/30 backdrop-blur-md' : isTactical ? 'bg-black/80 border border-ops-green/30' : 'bg-gray-900 text-white'}
                `}>
                    <h3 className={`text-xl font-bold mb-6 flex items-center gap-2 border-b pb-4 ${isOcean ? 'border-ocean-neon/30 text-ocean-neon' : isTactical ? 'border-ops-green/30 text-ops-green' : 'border-gray-700 text-gray-300'}`}>
                    ...
                    {jsonSpec ? (
                        <div className="relative flex-1 overflow-hidden rounded-xl bg-black/50 border border-white/5">
                             <pre className={`p-4 text-xs md:text-sm font-mono overflow-auto h-full max-h-[700px] custom-scrollbar ${isOcean ? 'text-cyan-400' : isTactical ? 'text-green-400' : 'text-green-400'}`}>
                                <code>{jsonSpec}</code>
                             </pre>
                             <div className="absolute top-2 right-2">
                                 <button 
                                    onClick={() => navigator.clipboard.writeText(jsonSpec)}
                                    className={`text-xs px-2 py-1 rounded hover:bg-white/10 transition-colors ${isOcean ? 'text-ocean-neon' : isTactical ? 'text-ops-green' : 'text-white'}`}
                                 >
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6 opacity-70">
                 {[
                    ...
                    { icon: 'fa-chart-line-down', title: 'TCO Reduction', desc: 'วิเคราะห์ต้นทุนรวมตลอดอายุการใช้งานเพื่อลดรายจ่าย' }
                     ...
                     <div key={idx} className={`p-6 rounded-2xl text-center border border-dashed border-transparent hover:border-current transition-all ${isOcean ? 'text-ocean-neon hover:bg-ocean-neon/5' : isTactical ? 'text-ops-green hover:bg-ops-green/5' : 'text-gray-500 hover:bg-gray-50'}`}>
